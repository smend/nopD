            ...
            out decimal DiductAmountInclTax)
        {
            ...
            foreach (var discount in allowedDiscounts)
            {
                ...
                foreach (var shoppingCartItem in productsForDeduction)
                {
                    ...
                    tempProducts++;
                    incTemp.Add(_taxService.GetProductPrice(shoppingCartItem.Product, _priceCalculationService.GetSubTotal(shoppingCartItem), true, customer, out stubTax));
                    exlTemp.Add(_taxService.GetProductPrice(shoppingCartItem.Product, _priceCalculationService.GetSubTotal(shoppingCartItem), false, customer, out stubTax));
                }
                incTemp.Sort();
                exlTemp.Sort();
